...
sex_input = st.selectbox("Sex", ["Male", "Female"])  # User sees "Male" or "Female"
sex = 1 if sex_input == "Male" else 0  # Map "Male" to 1 and "Female" to 0
...
if st.button("Predict"):
    prediction = model.predict(input_data)[0]  # Use predict() for regression
    st.success(f"Predicted Memory Score: {prediction:.2f}")
